...
class MethodReturnFunction {
  ...
  // Int into some type A, so you write:
  def map(f: (Int) => A)
  ...
  //That takes care of the method signature. Now all you have to do is write the method body. A map method applies 
  // the function it’s given to every element in the list it’s given to produce a new, transformed list. 
